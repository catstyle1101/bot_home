...
async def get_name(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        for i in scrapping_torrents_list:
            mess += str(i[0]) + '\t' + str(i[1]) + '\t' + '/download_'  + str(i[0]) + '\t' + i[2] + '\t' + i[4] +'\n' + i[3]
        await message.reply(mess)
    except:
        await message.reply('Похоже сеть или сайт недоступны, либо перефразируй запрос')
    await state.finish()
...
async def find_magnet(message: types.Message):
    ...
    if message.from_user.id in allowed_users:
        await message.reply(magnet + f'\n\n{torrent_name}\t{ torrent_size}\n\nСкачать файл?', reply_markup=inline_add_torrent_kb())
    else:
...
async def start_download(callback: types.CallbackQuery):
    ...
    if  res[1] == 'yes':
        await callback.answer(transmission_client.add_torrent(magnet, torrent_name), show_alert=True)
    else:
...
def register_handlers_scrapping(dp : Dispatcher):
    #dp.register_message_handler(start, commands='Найти_торрент', state=None)
    dp.register_callback_query_handler(start, Text(startswith='menu_find'), state=None)
